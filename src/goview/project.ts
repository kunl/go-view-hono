...
route.post('/create', async (c) => {
	...
	const projectCount = await db
		...
		.where(eq(schema.projects.createUserId, createUserId));
	...
	if (
		projectCount[0].count >= PROJECT_COUNT_LIMIT &&
		!unLimitUser.includes(createUserId.toString())
	) {
...
route.delete('/delete', async (c) => {
	...
	await db
		.delete(schema.projectDatas)
		.where(eq(schema.projectDatas.projectId, id));
...
route.get('/getData', async (c) => {
	...
	const db = drizzle(c.env.DB, { schema });
	if (projectId) {
		const res = await db.query.projectDatas.findFirst({
			where: eq(schema.projectDatas.projectId, projectId),
		});
